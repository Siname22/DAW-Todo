...
//--------------------------------Conceptos--------------------------------------------------
/*
Creamos clases orientadas a ConceptosObtenerTodos.php
 */
class Conceptos{
...
//Creamos un metodo que muestre los datos del Id por el nombre del concepto
    getNombrePorId(id){
...
function iniciar () {
    cogerConcepto();
    document.getElementById("change").addEventListener("click", cogerDato, false);
}
//--------------------------------------------------CONCEPTOS-----------------------------------------------------------
var conceptos;
...
function cogerDato(){
    ...
    req.onreadystatechange = function (){
        if (this.readyState === 4 && this.status === 200){
            ...
            crearTabla(obtenerObjetosValidos(validaSelect(), movimientos));
        }
    }
...
function validaSelect(){
    ...
    let selectData = document.getElementById("datos");
    for (var i=0, iLen=selectData.options.length; i<iLen; i++) {
        if (selectData.options[i].selected) {
            validacion.push(selectData.options[i].value);
        }
    }
...
/*
Coge el dato retornado de validaCheckBox() y lo utiliza para saber que datos mostrar en la vista al usuario. Retorna los
...
function obtenerObjetosValidos(tipo, movimientos){
    ...
    error.innerHTML = "";
    if(tipo.length === 0){
        error.innerHTML = "Error, selecciona algún select";
    }else{
